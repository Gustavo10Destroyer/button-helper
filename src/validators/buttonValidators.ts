...
const sendButtonsAllowedComplexNames = new Set<SendButtonsComplexName>([
    ...
    InteractiveButtonName.QuickReply,
]);

const interactiveAllowedNames = new Set<InteractiveButtonName>(
    Object.values(InteractiveButtonName),
);
        ...
        interactiveButtons: [
            {
                ...
                params: { displayText: 'Hello', id: 'hello' },
            },
            {
                name: InteractiveButtonName.CtaCopy,
                params: { displayText: 'Copy Code', copyCode: 'ABC123' },
            },
        ],
        footer: 'Footer',
    },
...
export function validateAuthoringButtons(
    buttons: unknown,
): ValidationResult<AuthoringButton[]> {
    const errors: string[] = [];
    ...
    if (buttons.length === 0) {
        ...
        warnings.push(
            `buttons count (${buttons.length}) exceeds soft cap of ${softButtonCap}; may be rejected by client`,
        );
    }
    ...
    buttons.forEach((button, index) => {
        ...
        if (
            isTypedNativeFlowButton(button) ||
            isSerializedNativeFlowButton(button) ||
            isLegacyBaileysButton(button) ||
        ...
        warnings.push(
            `button[${index}] unrecognized shape; it cannot be converted safely`,
        );
    });
...
export function validateSendButtonsPayload(data: unknown): ValidationResult {
    ...
    if (!Array.isArray(data.buttons) || data.buttons.length === 0) {
        ...
        data.buttons.forEach((button, index) =>
            validateSendButtonsButton(button, index, errors),
        );
    }
 ...
 */
export function validateSendInteractiveMessagePayload(
    data: unknown,
): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (
        ...
        data.interactiveButtons.forEach((button, index) =>
            validateInteractiveAuthoringButton(button, index, errors),
        );
    }
 ...
 */
export function validateInteractiveMessageContent(
    content: unknown,
): ValidationResult {
    const errors: string[] = [];
    const warnings: string[] = [];
    ...
    if (!isRecord(interactive)) {
        return {
            valid: false,
            errors: ['interactiveMessage must be an object'],
            warnings,
        };
    }
    ...
    nativeFlow.buttons.forEach((button, index) => {
        if (!isSerializedNativeFlowButton(button)) {
            errors.push(
                `buttons[${index}] must include a supported name and string buttonParamsJson`,
            );
            return;
        }
        ...
        try {
            ...
            const message =
                error instanceof Error
                    ? error.message
                    : 'Unknown buttonParamsJson validation error';
...
function validateSendButtonsButton(
    button: unknown,
    index: number,
    errors: string[],
): void {
    ...
    if (isTypedNativeFlowButton(button)) {
        if (
            !sendButtonsAllowedComplexNames.has(
                button.name as SendButtonsComplexName,
            )
        ) {
            errors.push(
                `button[${index}] name '${button.name}' not allowed in sendButtons`,
            );
        }
        return;
    }
    ...
    if (isSerializedNativeFlowButton(button)) {
        ...
        validateSerializedButtonParams(
            button.name,
...
function validateInteractiveAuthoringButton(
    button: unknown,
    index: number,
    errors: string[],
): void {
    ...
    if (typeof button.buttonParamsJson !== 'string') {
        errors.push(
            `interactiveButtons[${index}] buttonParamsJson must be string when params are not provided`,
        );
        return;
    }
...
function validateSerializedButtonParams(
    ...
    try {
        ...
        const message =
            error instanceof Error
                ? error.message
                : 'Unknown button parameter validation error';
