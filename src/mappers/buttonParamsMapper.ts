...
import { parseJson, requireRecord, requireStringField, optionalStringField } from '../parsers/jsonParser.js';
 ...
 */
export function serializeNativeFlowButton(button: NativeFlowButton): SerializedNativeFlowButton {
  return {
    name: button.name,
    buttonParamsJson: JSON.stringify(toExternalButtonParams(button.name, button.params)),
  };
}
 ...
 */
export function deserializeNativeFlowButton(button: SerializedNativeFlowButton): NativeFlowButton {
  return {
    name: button.name,
...
export function parseButtonParamsJson<TName extends InteractiveButtonName>(
  ...
  const record = requireRecord(parseJson(buttonParamsJson, `buttonParamsJson for ${name}`), `buttonParamsJson for ${name}`);

  switch (name) {
    case InteractiveButtonName.QuickReply:
      return {
        displayText: requireStringField(record, fieldKeys.displayText, name),
    ...
    case InteractiveButtonName.CtaCall:
      return {
        displayText: requireStringField(record, fieldKeys.displayText, name),
        phoneNumber: requireStringField(record, fieldKeys.phoneNumber, name),
      } as ButtonParamsByName[TName];
    case InteractiveButtonName.CtaCatalog:
      return {
        businessPhoneNumber: requireStringField(record, fieldKeys.businessPhoneNumber, name),
      } as ButtonParamsByName[TName];
    ...
    case InteractiveButtonName.MultiProductMessage:
      return {
        productId: requireStringField(record, fieldKeys.productId, name),
      } as ButtonParamsByName[TName];
    case InteractiveButtonName.PaymentTransactionDetails:
      return {
        transactionId: requireStringField(record, fieldKeys.transactionId, name),
      } as ButtonParamsByName[TName];
    case InteractiveButtonName.AutomatedGreetingCatalog:
      return {
        businessPhoneNumber: requireStringField(record, fieldKeys.businessPhoneNumber, name),
        catalogProductId: requireStringField(record, fieldKeys.catalogProductId, name),
      } as ButtonParamsByName[TName];
    case InteractiveButtonName.GalaxyMessage:
      return {
        flowToken: requireStringField(record, fieldKeys.flowToken, name),
        flowId: requireStringField(record, fieldKeys.flowId, name),
      } as ButtonParamsByName[TName];
    case InteractiveButtonName.SingleSelect:
      return parseSingleSelectParams(record, name) as ButtonParamsByName[TName];
 ...
 */
export function toExternalButtonParams(name: InteractiveButtonName, params: ButtonParams): JsonObject {
  switch (name) {
    case InteractiveButtonName.QuickReply: {
    ...
    case InteractiveButtonName.CtaUrl: {
      ...
      return { [fieldKeys.displayText]: typed.displayText, url: typed.url };
    }
    case InteractiveButtonName.CtaCopy: {
      const typed = params as CtaCopyParams;
      return { [fieldKeys.displayText]: typed.displayText, [fieldKeys.copyCode]: typed.copyCode };
    }
    case InteractiveButtonName.CtaCall: {
      const typed = params as CtaCallParams;
      return { [fieldKeys.displayText]: typed.displayText, [fieldKeys.phoneNumber]: typed.phoneNumber };
    }
    case InteractiveButtonName.CtaCatalog: {
      const typed = params as CatalogParams;
      return { [fieldKeys.businessPhoneNumber]: typed.businessPhoneNumber };
    }
    case InteractiveButtonName.CtaReminder:
    case InteractiveButtonName.CtaCancelReminder:
    case InteractiveButtonName.AddressMessage:
    ...
    case InteractiveButtonName.GalaxyMessage: {
      ...
      return { [fieldKeys.flowToken]: typed.flowToken, [fieldKeys.flowId]: typed.flowId };
    }
    case InteractiveButtonName.SingleSelect:
...
function parseSingleSelectParams(record: Record<string, unknown>, context: string): SingleSelectParams {
  ...
  return {
    ...
    sections: sectionsValue.map((section, sectionIndex) => parseSingleSelectSection(section, `${context}.sections[${sectionIndex}]`)),
  };
}

function parseSingleSelectSection(value: unknown, context: string): SingleSelectSection {
  ...
  return {
    title: requireStringField(record, 'title', context),
    rows: rowsValue.map((row, rowIndex) => parseSingleSelectRow(row, `${context}.rows[${rowIndex}]`)),
  };
}

function parseSingleSelectRow(value: unknown, context: string): SingleSelectRow {
  const record = requireRecord(value, context);
