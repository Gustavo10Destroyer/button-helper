 ...
 */
export function getButtonType(message: WAMessageContent | undefined): ButtonMessageType | null {
  if (!message) {
    return null;
  }
...
export function getButtonArgs(message: WAMessageContent): BinaryNode {
  ...
  if (firstButtonName === InteractiveButtonName.ReviewAndPay || firstButtonName === InteractiveButtonName.PaymentInfo) {
    return {
      tag: 'biz',
      attrs: {
        [binaryAttributeKeys.nativeFlowName]: firstButtonName === InteractiveButtonName.ReviewAndPay ? 'order_details' : firstButtonName,
      },
    };
  }
...
function isSpecializedNativeFlowName(name: string | null | undefined): name is InteractiveButtonName {
  return typeof name === 'string' && isInteractiveButtonName(name) && specializedNativeFlows.has(name);
}
